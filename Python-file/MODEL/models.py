    ...
    def start(self, threadName = None):
        try:
            ...
            for device in self.devices:
                ...
                self.client[d_id].config(ip_address=device.address, port=80, auth_user='admin', auth_pwd='scadatest1234', onvif_user='onvif', onvif_pwd='scadatest1234')
                    ...
                    for t in self.device_dict[d]:
                        if t == 'image':
                            ...
                            if count % 20 != 1:
                               ...
                               tag = self.device_dict[d][t]
                               if ret:
                                   res = loop.run_until_complete(pub.set('tag:'+str(tag.full_name), image_base64)) #await 
                                   print(tag.name+" : "+str('capture complete'))
